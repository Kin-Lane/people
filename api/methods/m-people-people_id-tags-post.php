...
$app->post($route, function ($people_id)  use ($app){
	...
	if(isset($param['tag']))
		...
		$CheckTagPivotQuery = "SELECT * FROM profile_tag_pivot where Tag_ID = " . trim($tag_id) . " AND Profile_ID = " . $people_id;
		...
		if($CheckTagPivotResult && mysql_num_rows($CheckTagPivotResult))
			...
			{
			$query = "INSERT INTO profile_tag_pivot(Tag_ID,Profile_ID) VALUES(" . $tag_id . "," . $people_id . "); ";
			mysql_query($query) or die('Query failed: ' . mysql_error());
			}
